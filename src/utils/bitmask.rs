...
impl Bitmask {
    pub fn from_dim2d(
        ...
        Self {
            size,
            word_num,
            words: vec![word; word_num],
        }
    }

    pub fn from_cube_log2dim(
        log2dim: usize,
        active: bool,
    ) -> Self {
        let mut word: usize = 0;
    ...
    pub fn get_next_bit_on(
        ...
        if word & ((1 as usize) << m) > 0 {
            return start; // simple case: start is active
        }
    
        word &= ((usize::MAX >> m) << m); // bitmask out lower bits
        while word == 0 {
            n += 1;
            if n == self.word_num {
                break;
